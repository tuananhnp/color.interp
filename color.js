...
var lab = new ColorSpace({
	...
	fromBase (XYZ) {
		...
		// now compute f
		let f = xyz.map(value => value > ε$4 ? Math.cbrt(value) : (κ$2 * value + 16) / 116);
 ...
 */
function inGamut (color, space = color.space, {epsilon = ε$3} = {}) {
	color = getColor(color);
	space = ColorSpace.get(space);
 ...
 * @returns {Color}
 */
function toGamutCSS (origin, { space = origin.space }) {
	const JND = 0.02;
	const ε = 0.0001;
	space = ColorSpace.get(space);
...
function toLstar (y) {
	...
	const fy = (y > ε$1) ? Math.cbrt(y) : (κ * y + 16) / 116;
